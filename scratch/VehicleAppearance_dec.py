...
# Compiled at: 2011-06-08 10:22:23
import BigWorld, ResMgr, Math, Pixie, weakref
from constants import IS_DEVELOPMENT
from debug_utils import *
from helpers import bound_effects
from helpers import DecalMap
from helpers.EffectsList import EffectsListPlayer
import items.vehicles, random, math
from itertools import chain
from Event import Event
from functools import partial
import material_kinds, Avatar
...
class VehicleAppearance(object):
    ...
    def prerequisites(self, vehicle):
        ...
        if vDesc.camouflageID is not None and customization is not None:
            camouflageId = vDesc.camouflageID
    ...
    def showStickers(self, bool):
        ...
        for (compName, stickerDesc) in self.__stickers.iteritems():
            ...
            stickerDesc['stickerModel'].setAlphas(alpha, alpha)
    ...
    def changeVisibility(self, modelName, modelVisible, attachmentsVisible):
        ...
        desc['_visibility'] = (
         modelVisible, attachmentsVisible)
    ...
    def changeEngineMode(self, mode):
        ...
        if sound is not None:
            ...
            if param is not None:
                ...
                self.__engineSndVariation = [
                 BigWorld.time() + 1.0, False]
        return
        ...
        for stickerDesc in self.__stickers.itervalues():
            ...
            if dmgSticker is not None:
                if dmgSticker['handle'] is not None:
                    stickerDesc['stickerModel'].delSticker(dmgSticker['handle'])
    ...
    def addDamageSticker(self, code, componentName, stickerID, segStart, segEnd):
        ...
        handle = stickerDesc['stickerModel'].addSticker(1, texCoords, model, segStart, segEnd, desc['modelSizes'], up)
    ...
    def enableOrDisableAllSwinging(self):
        self.__fashion.disableAllSwinging = False
        if self.__vehicle.isPlayer:
            player = BigWorld.player()
            if player.inputHandler is not None:
                if player.inputHandler.ctrl == player.inputHandler.ctrls['sniper']:
                    self.__fashion.disableAllSwinging = not BigWorld.wg_isSniperModeSwingingEnabled()
        return
    ...
    def __updateCamouflage(self):
        ...
        defaultTiling = None
        if vDesc.camouflageID is not None and customization is not None:
            camouflage = customization['camouflages'].get(vDesc.camouflageID)
        ...
        for descId in ('chassis', 'hull', 'turret', 'gun'):
            ...
            if tgDesc is not None:
                if tgDesc.has_key('camouflageTiling'):
                    if tiling is not None:
                        coeff = tgDesc['camouflageTiling']
                        tiling = (tiling[0] * coeff[0], tiling[1] * coeff[1], tiling[2] * coeff[2], tiling[3] * coeff[3])
                    else:
                        tiling = tgDesc['camouflageTiling']
            ...
            if param is not None:
                ...
                if time >= self.__engineSndVariation[0]:
                    if powerMode >= 3.0:
                        ...
                        self.__engineSndVariation[0] = time + random.choice((0.5, 0.25,
                                                                             0.75,
                                                                             0.25))
    ...
    def __updateEffectsLOD(self):
        ...
        disableDust = self.__distanceFromPlayer > _LOD_DISTANCE_DUST or not BigWorld.wg_isVehicleDustEnabled()
    ...
    def __updateCurrTerrainMatKinds(self):
        ...
        for iTrack in xrange(2):
            ...
            res = BigWorld.wg_collideSegment(self.__vehicle.spaceID, testPoint + (0,
                                                                                  2,
                                                                                  0), testPoint + (0,
                                                                                                   -2,
                                                                                                   0), 16)
    ...
    def __updateDustTrails(self):
        if self.__dustDisabled:
            ...
            if self.__vehicle.typeDescriptor.chassis['effects'] is None:
                ...
                for node in (centerNode, activeCornerNode, inactiveCornerNode):
                    ...
                    if nodeEffects is not None:
                        for nodeEffect in nodeEffects:
                            stopParticles = nodeEffect[1] != currMatKind or node == inactiveCornerNode
                            relEmissionRate = 0
                            if not self.__isInWater:
                                relEmissionRate = nodeEffect[2] + (-0.25 if stopParticles else 0.25)
    ...
    def executeRammingVibrations(self):
        if self.__vibrationsCtrl is not None:
            self.__vibrationsCtrl.executeRammingVibrations(self.__vehicle.getSpeed())
        return
    ...
    def __createDustParticleSystemIfNeeded(self, node, iTrack, effectGroup, currMatKind, drawOrder):
        ...
        if nodeEffects is None:
            ...
            nodeEffects.append([pixie, currMatKind, 1, 1, basicRates])
            return
    ...
    def __createStickers(self):
        ...
        emblemDescr = playerEmblems['descrs'][vDesc.playerEmblemID]
        (emblemTexName, emblemTexX, emblemTexY) = emblemDescr['texture']
        emblemTex = BigWorld.PyTextureProvider(emblemTexName)
        emblemTexSize = playerEmblems['textureSize']
        emblemTexX *= emblemTexSize
        emblemTexY *= emblemTexSize
        emblemTexCoords = Math.Vector4(emblemTexX / float(emblemTex.width), emblemTexY / float(emblemTex.height), emblemTexSize / float(emblemTex.width), emblemTexSize / float(emblemTex.height))
        emblemPositions = (
         (
          'hull', vDesc.hull['emblemSlots'][0] + vDesc.hull['emblemSlots'][1]),
         (
          'turret', vDesc.turret['emblemSlots'][0] + vDesc.turret['emblemSlots'][1]),
         (
          'gun', ()))
        for (componentName, slots) in emblemPositions:
            stickerModel = BigWorld.WGStickerModel(emblemTexName, damageTexName)
            stickerModel.setLODDistances(vDesc.type.emblemsLodDist, vDesc.type.damageStickersLodDist)
            self.__stickers[componentName] = {'stickerModel': stickerModel, 
               'emblemSlots': slots, 
               'emblemTexCoords': emblemTexCoords, 
               'damageStickers': {}, 'alpha': 1.0}
    ...
    def __attachStickers(self, alpha=1.0, emblemsOnly=False):
        ...
        for (componentName, stickerDesc) in self.__stickers.iteritems():
            stickerModel = stickerDesc['stickerModel']
            stickerModel.setAlphas(actualAlpha, actualAlpha)
            stickerDesc['alpha'] = actualAlpha
            model = self.modelsDesc[componentName]['model']
            node = self.modelsDesc[componentName]['_node']
            node.attach(stickerModel)
            for (rayStart, rayEnd, rayUp, size, hideIfDamaged) in stickerDesc['emblemSlots']:
                if hideIfDamaged and isDamaged:
                    continue
                sizes = Math.Vector2(size, size)
                stickerModel.addSticker(0, stickerDesc['emblemTexCoords'], model, rayStart, rayEnd, sizes, rayUp)

            if emblemsOnly:
                continue
            for dmgSticker in stickerDesc['damageStickers'].itervalues():
                if dmgSticker['handle'] is not None:
                    stickerModel.delSticker(dmgSticker['handle'])
                    dmgSticker['handle'] = None
                    LOG_WARNING('Adding %s damage sticker to occupied slot' % componentName)
                dmgSticker['handle'] = stickerModel.addSticker(1, dmgSticker['texCoords'], model, dmgSticker['rayStart'], dmgSticker['rayEnd'], dmgSticker['sizes'], dmgSticker['rayUp'])
    ...
    def __detachStickers(self):
        for (componentName, stickerDesc) in self.__stickers.iteritems():
            stickerModel = stickerDesc['stickerModel']
            node = self.modelsDesc[componentName]['_node']
            node.detach(stickerModel)
            stickerModel.clear(0)
            stickerModel.clear(1)
            for dmgSticker in stickerDesc['damageStickers'].itervalues():
                dmgSticker['handle'] = None
...
class _SkeletonCollider():
    ...
    def __init__(self, vehicle, vehicleAppearance):
        ...
        descList = [
         (
          'Scene Root', descr.chassis['hitTester'].bbox),
         (
    ...
    def __computeVehicleHeight(self):
        ...
        ret = desc.chassis['hullPosition'][1] + desc.hull['turretPositions'][0][1] + turretBBox[1][1]
        return max(ret, desc.chassis['hullPosition'][1] + hullBBox[1][1])
...
def _createWheelsListByTemplate(startIndex, template, count):
    return [_[1] for i in range(startIndex, startIndex + count)]
...
def _validateCfgPos(srcModelDesc, dstModelDesc, cfgPos, paramName, vehicle, state):
    ...
    if length > 0.01 and not _almostZero(realOffset.length):
        ...
        LOG_WARNING('%s parameter is incorrect. \n Note: it must be <%s>.\nPlayer: %s; Model: %s' % (
         paramName, realOffset, vehicle.publicInfo['name'], modelState))
...
# okay decompiling C:\Users\qwerty\Desktop\World_of_Tanks\res\scripts\client\VehicleAppearance.pyc
