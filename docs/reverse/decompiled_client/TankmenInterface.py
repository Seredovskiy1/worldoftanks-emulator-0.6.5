...
import time
from gui.Scaleform.utils.gui_items import formatPrice, formatTankmanPrice, getItemByCompact, makeTooltip, getTankmanOpertnSysMessageType
from AccountCommands import LOCK_REASON
from account_helpers.AccountPrebattle import AccountPrebattle
from gui.Scaleform.utils.dossiers_utils import TANKMEN_MEDALS_BLOCKS, getDossierMedals
import dossiers
...
class TankmenInterface(UIInterface):
    TANKMEN_ORDER = {'commander': 0, 
       ...
       'radioman': 3, 
       'loader': 4}
    STATS_BLOCKS = {'tankman': (
                 (
                  'common', ('battlesCount', 'wins', 'losses', 'survivedBattles')),
                 (
                  'battleeffect', ('frags', 'maxFrags')),
                 (
                  'credits', ('xp', 'avgExperience', 'maxXP'))), 
       'medals': ('warrior', 'invader', 'sniper', 'defender', 'steelwall', 'supporter', 'scout', 'medalKay',
 'medalCarius', 'medalKnispel', 'medalPoppel', 'medalAbrams', 'medalLeClerc', 'medalLavrinenko',
 'medalEkins', 'medalWittmann', 'medalOrlik', 'medalOskin', 'medalHalonen', 'medalBurda',
 'medalBillotte', 'medalKolobanov', 'medalFadin', 'beasthunter', 'titleSniper', 'invincible',
 'diehard', 'raider', 'handOfDeath', 'armorPiercer', 'kamikaze', 'lumberjack')}
    ...
    def __buyTankmanProcess(self, nationID, vehicleTypeID, role, tmanCostTypeIdx, slot, callback):
        ...
        def buyResponse(code, tmanInvID, tmanCompDescr):
            if code >= 0:
                callback((True,
                 makeString('#system_messages:buyTankman/success') + formatTankmanPrice(upgradeParams, tmanCostTypeIdx), tmanInvID))
            else:
    ...
    def __getData(self, item, dossier):
        if item == 'effectiveShots':
            if dossier['shots'] != 0:
                return '%d%%' % int(float(dossier['hits']) / dossier['shots'] * 100)
            return '0%'
        if item == 'avgExperience':
            if dossier['battlesCount'] != 0:
                return BigWorld.wg_getIntegralFormat(float(dossier['xp']) / dossier['battlesCount'])
            return 0
        return BigWorld.wg_getNiceNumberFormat(dossier[item])

    def __getExtra(self, block, item, dossier, isTotal=False):
        extra = ''
        if block == 'common':
    ...
    def onGetTankmanStats(self, callbackID, tankmanID):
        ...
        for block in self.STATS_BLOCKS['tankman']:
            ...
            for item in block[1]:
                stats.append(item)
                stats.append(self.__getData(item, dossier))
                stats.append(self.__getExtra(block[0], item, dossier))
    ...
    def __updateSkillsList(self, tankman):
        Waiting.show('updating')
        data = [
         makeString('#dialogs:addSkillWindow/label') % makeString('#dialogs:addSkillWindow/label/' + tankman.descriptor.role)]
        new_skill = len(tankman.skills) == 0 and tankman.roleLevel == 100 or tankman.lastSkillLevel == 100
        for skill in SKILL_NAMES:
            if skill in COMMON_SKILLS and skill not in tankman.skills:
                data.append(skill)
    ...
    def __showRecruitWindow(self):
        ...
        upgradeParams = yield StatsRequester().getTankmanCost()
        data = [
         credits, gold, round(upgradeParams[1]['credits']), round(upgradeParams[2]['gold'])]
    ...
    def __populateRetrainingWindow(self, tankman):
        ...
        if upgradeParams:
            ...
            for module in modulesAll:
                compdecs = module.descriptor.type.compactDescr
                for role in module.descriptor.type.crewRoles:
                    if tankman.descriptor.role == role[0]:
                        if compdecs in unlocks and module.descriptor.type.id[0] == tankman.nation and module.descriptor.type.id[1] not in data:
                            data.append(module.descriptor.type.id[1])
                            data.append(module.type)
                            data.append(module.descriptor.type.shortUserString)

            data[:4] = (
             upgradeParams[2]['gold'], upgradeParams[1]['credits'], gold, credits)
