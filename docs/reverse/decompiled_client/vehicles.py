...
import struct, nations, items
from items import _xml
from debug_utils import *
from constants import IS_DEVELOPMENT, IS_CLIENT, IS_CELLAPP, IS_BASEAPP, ITEM_DEFS_PATH
if not IS_BASEAPP:
...
VEHICLE_MODULE_TAGS = frozenset(('vehicle', 'gun', 'turret', 'engine', 'chassis', 'radio'))
NUM_OPTIONAL_DEVICE_SLOTS = 3
NUM_EQUIPMENT_SLOTS = 3
HORN_COOLDOWN_TIME = 1.0
...
class VehicleDescr(object):
    ...
    def __set_camouflageID(self, id):
        if id is not None:
            ...
            if descr['allow'] and cd not in descr['allow']:
                raise Exception, 'camouflage is incompatible with vehicle'
        self.__camouflageID = id
        return

    camouflageID = property((lambda self: self.__camouflageID), __set_camouflageID)
    ...
    def makeCompactDescr(self):
        ...
        if self.__hornID is not None:
            flags |= 64
        if self.__camouflageID is not None:
            flags |= 128
    ...
    def __installGun(self, gunID, turretPosition):
        ...
        self.turrets[turretPosition] = (
         turretDescr, newGunDescr)
        self.__updateAttributes()
    ...
    def __initFromCompactDescr(self, compactDescr):
        ...
        try:
            ...
            self.optionalDevices = [_[1] for n in xrange(NUM_OPTIONAL_DEVICE_SLOTS)]
            n = NUM_OPTIONAL_DEVICE_SLOTS - 1
            while optionalDevicesMask:
                if optionalDevicesMask & 1:
                    self.optionalDevices[n] = g_cache.optionalDevices()[unpack('<H', cd[:2])[0]]
                    cd = cd[2:]
                optionalDevicesMask >>= 1
                n -= 1
            ...
            if not flags & 128:
                self.__camouflageID = None
            else:
                camID = unpack('<H', cd[:2])[0]
                cd = cd[2:]
                g_cache.customization(nationID)['camouflages'][camID]
                self.__camouflageID = camID
            self.__updateAttributes()
        except Exception:
    ...
    def __updateAttributes(self):
        ...
        if IS_CLIENT or IS_CELLAPP:
            ...
            if not IS_CLIENT or IS_DEVELOPMENT:
                self.physics['brakeForce'] = chassis['brakeForce']
                self.physics['terrainResistance'] = chassis['terrainResistance']
            ...
            if not IS_CLIENT or IS_DEVELOPMENT:
                bestResistance = physics['terrainResistance'][0]
                rotationEnergy = type.engines[0]['power'] * (weight / defWeight) / chassis['rotationSpeed']
                rotationSpeedLimit = physics['enginePower'] / rotationEnergy
                rotationEnergy /= bestResistance
                if not chassis['rotationIsAroundCenter']:
                    rotationEnergy -= trackCenterOffset * weight * physics['specificFriction'] / bestResistance
...
_dictDescrGetters = {'shell': (lambda nationID, compTypeID: g_cache.shells(nationID)[compTypeID]), 'equipment': (lambda nationID, compTypeID: g_cache.equipments()[compTypeID]), 
...
def makeIntCompactDescrByID(itemTypeName, nationID, compTypeID):
    header = items.ITEM_TYPE_INDICES[itemTypeName] + (nationID << 4)
    return (compTypeID << 8) + header
...
class VehicleType(object):
    ...
    def __init__(self, nationID, basicInfo, xmlPath):
        ...
        self.speedLimits = (
         KMH_TO_MS * _xml.readPositiveFloat(xmlCtx, section, 'speedLimits/forward'),
         KMH_TO_MS * _xml.readPositiveFloat(xmlCtx, section, 'speedLimits/backward'))
        ...
        self.crewSkillSlots = _computeCrewSkillSlots((xmlCtx, 'crew'), self.crewRoles)
        if IS_BASEAPP:
        ...
        if IS_CLIENT or IS_CELLAPP:
            ...
            self.tankmen = commonConfig['tankmen']
        if IS_CLIENT or IS_BASEAPP:
    ...
    def __convertAndValidateUnlocksDescrs(self, srcList):
        ...
        for descr in srcList:
            ...
            destList.append((descr[0], compactDescr) + descr[3:])
...
class Cache(object):
    ...
    def __init__(self):
        ...
        self.__vehicles = [_[1] for i in nations.NAMES]
        self.__chassis = [_[2] for i in nations.NAMES]
        self.__engines = [_[3] for i in nations.NAMES]
        self.__fuelTanks = [_[4] for i in nations.NAMES]
        self.__radios = [_[5] for i in nations.NAMES]
        self.__turrets = [_[6] for i in nations.NAMES]
        self.__guns = [_[7] for i in nations.NAMES]
        self.__shells = [_[8] for i in nations.NAMES]
        self.__optionalDevices = None
        self.__optionalDeviceIDs = None
        self.__equipments = None
        self.__equipmentIDs = None
        self.__vehicleIDs = [_[9] for i in nations.NAMES]
        self.__chassisIDs = [_[10] for i in nations.NAMES]
        self.__engineIDs = [_[11] for i in nations.NAMES]
        self.__fuelTankIDs = [_[12] for i in nations.NAMES]
        self.__radioIDs = [_[13] for i in nations.NAMES]
        self.__turretIDs = [_[14] for i in nations.NAMES]
        self.__gunIDs = [_[15] for i in nations.NAMES]
        self.__shellIDs = [_[16] for i in nations.NAMES]
        self.__customization = [_[17] for i in nations.NAMES]
        self.__horns = None
    ...
    def vehicle(self, nationID, vehicleTypeID):
        ...
        ct = VehicleType(nationID, basicInfo, xmlPath)
        self.__cont[id] = ct
        return ct
    ...
    def customization(self, nationID):
        ...
        if descr is None:
            descr = _readCustomization(_VEHICLE_TYPE_XML_PATH + nations.NAMES[nationID] + '/customization.xml', nationID)
    ...
    def _commonConfig(self):
        ...
        if self.__commonConfig is None:
            ...
            HORN_COOLDOWN_TIME = self.__commonConfig['miscParams']['hornCooldownTime']
            commonXml = None
    ...
    def __getList(self, nationID, listName):
        ...
        if not nations[nationID]:
            self.__readNation(nationID)
        return nations[nationID]

    def __readNation(self, nationID):
        compsXmlPath = _VEHICLE_TYPE_XML_PATH + nations.NAMES[nationID] + '/components/'
...
def _readChassis(xmlCtx, section, compactDescr, unlocksDescrs=None, parentItem=None):
    ...
    res['shotDispersionFactors'] = (
     _xml.readNonNegativeFloat(xmlCtx, section, 'shotDispersionFactors/vehicleMovement') / KMH_TO_MS,
     _xml.readNonNegativeFloat(xmlCtx, section, 'shotDispersionFactors/vehicleRotation') / radians(1.0))
    if not IS_CLIENT or IS_DEVELOPMENT:
        ...
        v = _xml.readVector3(xmlCtx, section, 'terrainResistance')
...
def _readGun(xmlCtx, section, compactDescr, unlocksDescrs=None, turretCompactDescr=None):
    ...
    if not section.has_key('burst'):
        ...
        count = _xml.readInt(xmlCtx, section, 'burst/count', 1)
        interval = 60.0 / _xml.readPositiveFloat(xmlCtx, section, 'burst/rate')
        res['burst'] = (count, interval if count > 1 else 0.0)
    nationID = parseIntCompactDescr(compactDescr)[1]
    v = []
...
def _readGunShotDispersionFactors(xmlCtx, section, subsectionName):
    return {'turretRotation': (_xml.readNonNegativeFloat(xmlCtx, section, subsectionName + '/turretRotation') / radians(1.0)), 
...
def _readArmor(xmlCtx, section, subsectionName):
    ...
    if IS_BASEAPP:
        ...
        for matKindName in section.keys():
            ...
            res[materialKind] = (_xml.readNonNegativeFloat(xmlCtx, section, matKindName),
             section.readBool(matKindName + '/noDamage', False))
...
def _computeCrewSkillSlots(xmlCtx, crewRoles):
    from items.tankmen import ROLES, COMMON_SKILLS
    zeros = (0, 0)
    res = dict.fromkeys(ROLES, zeros)
    for roles in crewRoles:
        isCommander = roles[0] == 'commander'
        for skillName in roles:
            (withCmd, withoutCmd) = res[skillName]
            withCmd += 1
            if not isCommander:
                withoutCmd += 1
            res[skillName] = (
    ...
    for skillName in ROLES:
        if res[skillName][0] == 0:
            _xml.raiseWrongXml(xmlCtx, '', "no tankman with role '%s'" % skillName)

    commonSkillSlots = (len(crewRoles), len(crewRoles) - 1)
    for skillName in COMMON_SKILLS:
        res[skillName] = commonSkillSlots
...
def _readEmblemSlots(xmlCtx, section, subsectionName):
    ...
    for (sname, subsection) in _xml.getChildren(xmlCtx, section, subsectionName):
        ...
        (playerSlots if sname == 'player' else clanSlots).append(descr)

    return (playerSlots, clanSlots)
...
def _readCommonConfig(xmlCtx, section):
    ...
    res['miscParams'] = {'projectileSpeedFactor': (_xml.readPositiveFloat(xmlCtx, section, 'miscParams/projectileSpeedFactor')), 
       'hornCooldownTime': (_xml.readNonNegativeFloat(xmlCtx, section, 'miscParams/hornCooldownTime')), 
    ...
    if IS_CLIENT or IS_CELLAPP:
        ...
        res['tankmen'] = _readExtrasSet(xmlCtx, section, 'tankmen', res['extrasDict'])
...
def _readMaterialDevices(xmlCtx, section, subsectionName, extrasDict):
    ...
    for (materialKindName, subsection) in _xml.getChildren(xmlCtx, section, subsectionName):
        ctx = (
         xmlCtx, subsectionName + '/' + materialKindName)
...
def _readCamouflage(xmlCtx, section, ids, groups, nationID):
    ...
    descr = {'groupNames': groupNames, 
       'gold': (_xml.readInt(xmlCtx, section, 'gold', 0)), 
    ...
    for (name, subsection) in section.items():
        ...
        if IS_CLIENT:
            ...
            if descr['mode'] == 'twoSounds':
                descr['sounds'] = (
                 _xml.readNonEmptyString(ctx, subsection, 'sound1'),
                 _xml.readNonEmptyString(ctx, subsection, 'sound2'))
            else:
                descr['sounds'] = (
                 _xml.readNonEmptyString(ctx, subsection, 'sound'),)
...
def _collectComponents(compactDescrs, compList):
    compactDescrs.update([_[1] for x in compList])
    return
