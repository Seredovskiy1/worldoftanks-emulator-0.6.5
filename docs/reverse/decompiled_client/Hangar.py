...
# Compiled at: 2011-06-17 14:30:54
...
import BigWorld, MusicController
from adisp import process
from CurrentVehicle import g_currentVehicle
from debug_utils import LOG_DEBUG, LOG_CURRENT_EXCEPTION
from items.tankmen import getSkillsConfig, COMMANDER_ADDITION_RATIO, COMMON_SKILLS, SKILL_NAMES
from items import ITEM_TYPE_NAMES
from helpers.i18n import convert, makeString
from gui import SystemMessages, AOGAS
from gui.Scaleform.graphs.VehicleComponentsGraphInterface import VehicleComponentsGraphInterface
...
from gui.Scaleform.utils.gui_items import formatPrice, getItemByCompact, makeTooltip
...
class Hangar(UIInterface):
    ...
    def __init__(self):
        ...
        self.componentsGrafInterface = VehicleComponentsGraphInterface()
    ...
    def populateUI(self, proxy):
        ...
        self.componentsGrafInterface.populateUI(proxy)
        self.componentsGrafInterface.onUnlocked += self.__onComponentUnlocked
        self.componentsGrafInterface.onComponentInstalled += self.__onComponentInstalled
        ...
        self.call('hangar.setCarouselFilter', [
         self.vehiclesFilter['nation'], self.vehiclesFilter['tankType'], self.vehiclesFilter['ready']])
    ...
    def dispossessUI(self):
        self.componentsGrafInterface.dispossessUI()
        self.componentsGrafInterface.onUnlocked -= self.__onComponentUnlocked
        self.componentsGrafInterface.onComponentInstalled -= self.__onComponentInstalled
        if self.__premiumHangarSndFan is not None:
    ...
    def onGetUnlockVehicles(self, callbackId):
        ...
        for shpVcl in shopVehicles:
            if shpVcl.descriptor.type.compactDescr in unlocks and shpVcl not in iventoryVehicles and not shpVcl.hidden:
                price = yield shpVcl.getPrice()
                if credits >= price[0] and gold >= price[1]:
                    value.append(shpVcl.pack())
    ...
    @process
    def onSellVehicle(self, callbackID, id, count):
        Waiting.show('sellVehicle')
        item = getItemByCompact(id)
        vcls = yield Requester('vehicle').getFromInventory()
        if item not in vcls:
            return
        success = yield item.sell(count=count)
        self.uiHolder.updateAccountInfo()
        self.update()
    ...
    def onGetSellVehicleParams(self, callbackID, compact):
        ...
        if vehicle is not None:
            ...
            devicesCount = 0
            for device in item.descriptor.optionalDevices:
            ...
            message = (makeString('#dialogs:sellConfirmation/messageCombineTankmen') + '\n') % ("<font color='#ff0000'>%d</font>" % crewCount) if crewCount > 0 else ''
            message += (makeString('#dialogs:sellConfirmation/messageCombineDevices') + '\n') % ("<font color='#ff0000'>%d</font>" % devicesCount) if devicesCount > 0 else ''
            message += (makeString('#dialogs:sellConfirmation/messageCombineEquipment') + '\n') % ("<font color='#ff0000'>%d</font>" % equipmentsCount) if equipmentsCount > 0 else ''
            message += '\n' + makeString('#dialogs:sellConfirmation/messageCombineQuestion') % (
             BigWorld.wg_getIntegralFormat(price[0]), " <img src='icon_credits' /> ")
            self.respond([callbackID, makeString('#dialogs:sellConfirmation/title') % item.name, message])
        return
    ...
    def update(self, diff=None):
        if diff is None or diff.get('inventory', {}).get(1, {}).get('compDescr', {}).has_key(g_currentVehicle.vehicle.inventoryId) or diff.get('inventory', {}).get(1, {}).get('repair', {}).has_key(g_currentVehicle.vehicle.inventoryId) or diff.get('inventory', {}).get(1, {}).get('shells', {}).has_key(g_currentVehicle.vehicle.inventoryId) or diff.get('inventory', {}).get(1, {}).get('shellsLayout', {}).has_key(g_currentVehicle.vehicle.inventoryId) or diff.get('inventory', {}).get(1, {}).get('eqs', {}).has_key(g_currentVehicle.vehicle.inventoryId) or diff.get('inventory', {}).get(1, {}).get('eqsLayout', {}).has_key(g_currentVehicle.vehicle.inventoryId):
            g_currentVehicle.update()
        elif diff.get('inventory', {}).get(1, False):
            self.__updateVehicles()
        elif diff.get('inventory', False) or 'credits' in diff.get('stats', {}) or 'gold' in diff.get('stats', {}):
    ...
    def __updateParams(self):
        self.call('hangar.setVehicleParameters', gui_items.getVehicleParams())
        return
    ...
    def __onBattleResultsReceived(self, isActiveVehicle=None, id=None, results=None):
        if id == g_currentVehicle.vehicle.inventoryId:
            ...
            self.__updateVehicles()
            self.__fitting.update()
        self.componentsGrafInterface.updateExperience()
        return

    def __updateLock(self, id, lock):
        if not (g_currentVehicle.vehicle and id == g_currentVehicle.vehicle.inventoryId):
            self.__updateVehicles()
        return
        ...
        if g_currentVehicle.isPresent():
            ...
            for tankman in tankmen:
                if not tankman.isInTank or tankman.inventoryId in vehicle.crew:
                    ...
                    if bonus_role_level != 0:
                        decrypt_role_level = '%s+%d' % (decrypt_role_level, bonus_role_level)
                    skills_list = ''
                    for skill in tankman.skills:
                        skills_list = '%s\n%s: %d%%' % (skills_list,
                         convert(getSkillsConfig()[skill]['userString']),
                         100 if tankman.skills.index(skill) + 1 != len(tankman.skills) else tankman.lastSkillLevel)

                    data.append(makeTooltip('%s %s %s' % (tankman.rank, tankman.firstname, tankman.lastname), '%s, %s %s, %d%% (%s)%s' % (tankman.role,
                     convert(makeString('#menu:tankmen/%s' % tankman.vehicleType)), convert(tankman.vehicle.type.shortUserString),
                     common_role_level, decrypt_role_level, skills_list), convert(makeString('#tooltips:hangar/crew/note'))))
                    skills_count = 0
    ...
    def __updateVehicles(self, resetPos=False):
        ...
        slots = yield StatsRequester().getSlotsCount()
        price = yield StatsRequester().getSlotsPrices()
        doubleXPVehs = yield StatsRequester().getDoubleXPVehicles()
        ...
        for vehicle in vehicles:
            ...
            try:
                ...
                v.append(vehicle.descriptor.type.compactDescr in doubleXPVehs)
                tooltip = makeTooltip(vehicle.longName, makeString('#tooltips:tanks_carousel/vehicleStates/%s' % vehicle.getState()), makeString('#tooltips:tanks_carousel/current_vehicle/note' if vehicle.isCurrent else '#tooltips:tanks_carousel/vehicle/note'))
                v.append(tooltip)
    ...
    def __updateAmmo(self):
        ...
        credits = yield StatsRequester().getCredits()
        ammo = [2, 3, False, False, 3, 3, False, False]
        if g_currentVehicle.isPresent():
            vehicle = g_currentVehicle.vehicle
            ...
            for shell in vehicle.shells:
                ...
                if shopShell:
                    ...
                    sPrice = (yield shopShell.getPrice()) if shell is not shopShell else (0,
                                                                                          0)
    ...
    def onAmmoInstall(self, callbackId, *args):
        ...
        if vehicle is not None:
            ...
            if len(shells) > 0:
                while len(shells) > 0:
                    item = gui_items.getItemByCompact(shells.pop(0))
    ...
    def __ammoBuyInstall(self, vehicle, buyAmmo, installAmmo):
        ...
        for (item, count) in buyAmmo.items():
            if count > 0:
                ...
                message = response[1]
                if not success:
                    if totalPrice['credits'] > 0 or totalPrice['gold'] > 0 and accountMoney['credits'] >= totalPrice['credits'] and totalPrice['gold'] >= totalPrice['gold']:
            ...
            if len(buyAmmo) > 0:
                price = (
                 totalPrice['credits'], totalPrice['gold'])
    ...
    def __onComponentUnlocked(self, vCompactDescr, uCompactDescr, usedXP):
        self.__fitting.update()
        return

    def __onComponentInstalled(self, vehicleDescr, itemTypeName, itemCompDescr):
        if itemTypeName in ('vehicleTurret', 'vehicleGun'):
...
class Fitting(UIInterface):
    ...
    def __requestAvailableItems(self, type):
        ...
        if type in Fitting.__ARTEFACTS_SLOTS:
            unlocks = [_[1] for m in data if m.isCurrent]
        else:
            unlocks = yield StatsRequester().getUnlocks()
        ...
        for module in data:
            ...
            fits = gui_items.isModuleFitVehicle(module, g_currentVehicle.vehicle, price, (credits, gold), unlocks)
            value.append(fits[1])
            value.append(createToolTip(module, fits[2]))
            if type in Fitting.__ARTEFACTS_SLOTS:
                fits = gui_items.isModuleFitVehicle(module, g_currentVehicle.vehicle, price, (credits, gold), unlocks, 1)
                value.append(fits[1])
                value.append(createToolTip(module, fits[2]))
                fits = gui_items.isModuleFitVehicle(module, g_currentVehicle.vehicle, price, (credits, gold), unlocks, 2)
                value.append(fits[1])
                value.append(createToolTip(module, fits[2]))
        ...
        if type in Fitting.__ARTEFACTS_SLOTS:
            ...
            value.append(0)
            value.append(cost)
            unlock_reasons = [_[2] for i in xrange(6)]
            for (i, module) in enumerate(unlocks):
                if module:
                    fits1 = gui_items.isModuleFitVehicle(module, g_currentVehicle.vehicle, (0,
                                                                                            0), (credits, gold), unlocks, module.index)
                    fits2 = gui_items.isModuleFitVehicle(module, g_currentVehicle.vehicle, (0, cost), (credits, gold), unlocks, module.index)
    ...
    def onFittingChange(self, callbackID, id, slotIdx=0, isUseGold=False):
        ...
        if isinstance(component, gui_items.ShopItem):
            ...
            if success and component is not None:
                self.uiHolder.updateAccountInfo()
        (success, message) = yield component.apply(slotIdx=int(slotIdx), isUseGold=isUseGold)
