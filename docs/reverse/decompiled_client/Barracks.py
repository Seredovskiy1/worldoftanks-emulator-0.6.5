...
from items.tankmen import TankmanDescr, getNationConfig, getSkillsConfig, ROLES
...
class Barracks(UIInterface):
    ...
    def onCheckBerthBuyDialog(self, callbackID):
        ...
        self.call('barracks.showBerthBuyDialog', [gold, berthPrice])
        return
    ...
    def onShowRecruitWindow(self, callbackID):
        ...
        data = [
         credits, gold, round(upgradeParams[1]['credits']), upgradeParams[2]['gold'], len(ROLES)]
        for role in ROLES:
            data.append(role)
    ...
    def __updateTankmen(self):
        ...
        for tankman in tankmen:
            ...
            data.append(gui_items.makeTooltip('%s %s %s' % (tankman.rank, tankman.firstname, tankman.lastname), '%s, %s %s %s' % (tankman.role, convert(makeString('#menu:tankmen/%s' % tankman.vehicleType)),
             convert(tankman.vehicle.type.shortUserString), skills_list), convert(makeString('#tooltips:hangar/crew/note'))))
