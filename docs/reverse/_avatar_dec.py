...
# Compiled at: 2011-06-22 09:44:02
import BigWorld, Keys, Math, cPickle
from functools import partial
import math, Vehicle, ClientArena, AvatarInputHandler, ProjectileMover, Settings, VehicleGunRotator, constants, Event, AreaDestructibles, CommandMapping, Weather, MusicController, SoundGroups, AvatarPositionControl, ResMgr
from constants import ARENA_PERIOD, AIMING_MODE, VEHICLE_SETTING, DEVELOPMENT_INFO, SHOT_RESULT
from constants import SERVER_TICK_LENGTH, AUTO_AIM_LOSS_REASON
...
class PlayerAvatar(BigWorld.Entity, ClientChat):
    ...
    def __init__(self):
        ...
        self.__rangeStreamIDCallbacks.addRangeCallback((
         STREAM_ID_CHAT_MIN, STREAM_ID_CHAT_MAX), '_ClientChat__receiveStreamedData')
    ...
    def onBecomePlayer(self):
        ...
        self.__deviceStates = {}
        self.__ownHornSoundCooldownTime = -1
            ...
            try:
                ...
                if cmdMap.isFired(CommandMapping.CMD_SWITCH_SERVER_MARKER, key) and isDown:
                    ...
                    if cmdMap.isFiredList((CommandMapping.CMD_MOVE_FORWARD,
                     CommandMapping.CMD_MOVE_FORWARD_SPEC,
                     CommandMapping.CMD_MOVE_BACKWARD), key) and isDown and not self.__isForcedGuiControlMode:
                        self.__cruiseControlMode = _CRUISE_CONTROL_MODE.NONE
                        self.__updateCruiseControlPanel()
                    if cmdMap.isFired(CommandMapping.CMD_STOP_UNTIL_FIRE, key) and isDown and not self.__isForcedGuiControlMode:
                        self.__stopUntilFire = self.__stopUntilFire or True
                ...
                if not self.__isForcedGuiControlMode and cmdMap.isFiredList(xrange(CommandMapping.CMD_AMMO_CHOICE_1, CommandMapping.CMD_AMMO_CHOICE_0 + 1), key) and isDown and mods == 0:
                    ...
                    return True
                if cmdMap.isFired(CommandMapping.CMD_CHAT_SHORTCAT_ATTACK_MY_TARGET, key) and isDown and mods == 0:
                    self.__onChatShortcutAttackMyTarget()
                    return True
                if cmdMap.isFired(CommandMapping.CMD_CHAT_SHORTCAT_ATTACK, key) and isDown and mods == 0:
                    self.__onChatShortcut('ATTACK')
                    return True
                if cmdMap.isFired(CommandMapping.CMD_CHAT_SHORTCAT_BACKTOBASE, key) and isDown and mods == 0:
                    self.__onChatShortcut('BACKTOBASE')
                    return True
                if cmdMap.isFired(CommandMapping.CMD_CHAT_SHORTCAT_FOLLOWME, key) and isDown and mods == 0:
                    self.__onChatShortcut('FOLLOWME')
                    return True
                if cmdMap.isFired(CommandMapping.CMD_CHAT_SHORTCAT_POSITIVE, key) and isDown and mods == 0:
                    self.__onChatShortcut('POSITIVE')
                    return True
                if cmdMap.isFired(CommandMapping.CMD_CHAT_SHORTCAT_NEGATIVE, key) and isDown and mods == 0:
                    self.__onChatShortcut('NEGATIVE')
                    return True
                if cmdMap.isFired(CommandMapping.CMD_CHAT_SHORTCAT_HELPME, key) and isDown and mods == 0:
    ...
    def onEnterWorld(self, prereqs):
        ...
        clrs = self.guiConfig['silhouetteColors']
    ...
    def onLeaveWorld(self):
        ...
        if self.__stepsTillInit == 0:
            try:
                ...
                SoundGroups.g_instance.unloadSounds('arena')
            except Exception:
                LOG_CURRENT_EXCEPTION()
    ...
    def updateVehicleHealth(self, health, isCrewActive):
        ...
        if not isAlive and wasAlive:
            ...
            battleWindow.showPostmortemTips()
    ...
    def updateVehicleAmmo(self, compactDescr, quantity, timeRemaining):
        ...
        if processor:
            getattr(self, processor)(compactDescr, quantity, timeRemaining)
        else:
    ...
    def updateTargetingInfo(self, turretYaw, gunPitch, maxTurretRotationSpeed, maxGunRotationSpeed, shotDispMultiplierFactor, aimingTime):
        self.__aimingInfo[2] = shotDispMultiplierFactor
        self.__aimingInfo[3] = aimingTime
        self.getOwnVehicleShotDispersionAngle(0.0)
    ...
    def showVehicleDamageInfo(self, damageIndex, extraIndex, entityID):
    ...
    def useHorn(self, start):
        if not self.__isVehicleAlive or not self.__isOnArena:
            ...
            if start and BigWorld.time() < self.__ownHornSoundCooldownTime:
                return
            ...
            if playerVehicle is not None:
                if start:
                    ...
                    self.__ownHornSoundCooldownTime = BigWorld.time() + vehicles.HORN_COOLDOWN_TIME + 0.2
                else:
    ...
    def prerequisites(self):
        ...
        if self.__fakeModelName:
            ...
            LOG_ERROR("The '%s' is missing or empty in '%s'" % (
             Settings.KEY_FAKE_MODEL, Settings.g_instance.scriptConfig.name))
    ...
    def getCurrentShots(self):
        shellCount = 0 if self.__currShellsIdx is None else self.__ammo[self.__currShellsIdx][1]
        return (shellCount, self.vehicleTypeDescriptor.shot)
    ...
    def getOwnVehicleShotDispersionAngle(self, turretRotationSpeed, isShot=False):
        ...
        (aimingStartTime, aimingStartFactor, multFactor, aimingTime) = self.__aimingInfo
        ...
        idealFactor = multFactor * math.sqrt(1 + idealFactor)
    ...
    def playShotResultNotification(self, result, target):
        if result is not None:
            if target.publicInfo.team != self.team:
                ...
                severity = 3
                g_windowsManager.battleWindow.pMsgsPanel.showMessage('ALLY_HIT', {'entity': (target.publicInfo.name)})
    ...
    __shotResultSeveritiesForEnemy = (
     1, 2, 2, 5, 6, 4)
    ...
    def __initGUIConfig(self):
        sc = Settings.g_instance.scriptConfig
        ...
        out['silhouetteColors'] = dict()
        colors = out['silhouetteColors']
        colors['self'] = sc.readVector4('silhouettesColors/self', Math.Vector4(0.2, 0.2, 0.2, 0.5))
        colors['enemy'] = sc.readVector4('silhouettesColors/enemy', Math.Vector4(1, 0, 0, 0.5))
        colors['friend'] = sc.readVector4('silhouettesColors/friend', Math.Vector4(0, 1, 0, 0.5))
        return out
    ...
    def __setVisibleGUI(self, bool):
        ...
        if self.gameInfoPanel is not None:
            self.gameInfoPanel.setVisible(bool)
        if self.playerBonusesPanel is not None:
            self.playerBonusesPanel.setVisible(bool)
        from gui.Scaleform.Battle import Battle
        if isinstance(g_windowsManager.window, Battle):
            g_windowsManager.window.setVisible(bool)
    ...
    def __setIsOnArena(self, onArena):
        if self.__isOnArena == onArena:
            ...
            onArena or self.gunRotator.stop()
        else:
    ...
    __damageInfoDestructions = (
     ...
     'TANKMAN_HIT', 'TANKMAN_HIT_AT_SHOT',
        ...
        if damageCode in self.__damageInfoNoMessage:
            ...
            if entityID != 0:
                ...
                if vehicleInfo is None:
                    ...
                    return
                names['entity'] = vehicleInfo['name']
                if vehicleInfo['clanAbbrev']:
                    names['entity'] += '[%s]' % vehicleInfo['clanAbbrev']
            g_windowsManager.battleWindow.vMsgsPanel.showMessage(damageCode, names)
            return

    __damageInfoNoMessage = (
    ...
    def __onArenaVehicleKilled(self, targetID, attackerID, reason):
        if targetID == self.playerVehicleID:
            ...
            if attackerID == self.playerVehicleID:
                ...
                playerMessagesPanel.showMessage(msg, {'target': (targetInfo['name'])})
                return
            targetInfo = self.arena.vehicles.get(targetID)
            ...
            if targetID == attackerID:
                ...
                playerMessagesPanel.showMessage(msg, {'entity': (targetInfo['name'])})
                return
            if attackerTeam == self.team:
                if attackerTeam != targetTeam:
                    msg = 'ally_frag'
                elif attackerInfo.get('isTeamKiller', False):
                    msg = 'teamkiller_friendly_fire_frag'
                else:
                    msg = 'ally_friendly_fire_frag'
            else:
                msg = 'enemy_frag' if attackerTeam != targetTeam else 'enemy_friendly_fire_frag'
            playerMessagesPanel.showMessage(msg, {'target': (targetInfo['name']), 'attacker': (attackerInfo['name'])})
            return
    ...
    def __onUserChatCommand(self, commandData):
        ...
        soundNotification = cmd.get('sound_notification')
        if soundNotification is not None and SoundGroups.g_instance.getEnableShortcutSounds():
        ...
        if cmd == CHAT_COMMANDS.ATTENTIONTOCELL:
            ...
            if markerName is not None:
                vehicle = self.__findNonPlayerVehicleByName(commandData['originatorNickName'])
                if vehicle is not None:
                    vmManager.showActionMarker(vehicle.marker, markerName)
            return

    def __findNonPlayerVehicleByName(self, name):
        for (vehID, vehInfo) in self.arena.vehicles.iteritems():
            if vehInfo['name'] == name:
    ...
    def __applyTimeAndWeatherSettings(self, overridePresetID=None):
        ...
        if len(presets) == 0 or presets[0].get('name') is None:
            ...
            try:
                ...
                weather.fog((fogColor[0], fogColor[1], fogColor[2], fogDensity))
    ...
    def __processVehicleEquipments(self, compactDescr, quantity, timeRemaining):
        ...
        if idx is not None:
            self.__equipment[idx] = (
             compactDescr, quantity, timeRemaining)
            consumablesPanel.setItemQuantityInSlot(idx, quantity)
...
# okay decompiling C:\Users\qwerty\Desktop\World_of_Tanks\res\scripts\client\Avatar.pyc
