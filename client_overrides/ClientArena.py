...
import Math, ResMgr, ArenaType
from items import vehicles
import constants, cPickle, Event
...
class ClientArena(object):
    ...
    def __onBasePointsUpdate(self, argStr):
        (team, baseID, points) = cPickle.loads(argStr)
        self.onTeamBasePointsUpdate(team, baseID, points)
        return

    def __onBaseCaptured(self, argStr):
        team = cPickle.loads(argStr)
        self.onTeamBaseCaptured(team)
        return
    ...
    def __vehicleInfoAsDict(self, info):
        infoAsDict = {'vehicleType': (vehicles.VehicleDescr(compactDescr=info[1])), 
           'name': (info[2]), 
           'team': (info[3]), 
           'isAlive': (info[4]), 
...
# okay decompiling c:\Users\qwerty\Desktop\World_of_Tanks\res\scripts\client\ClientArena.pyc
