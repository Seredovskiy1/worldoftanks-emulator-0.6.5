...
import BigWorld, Math, math, items, constants, ClientArena
...
class ProjectileMover(object):
    ...
    def add(self, shotID, effectsDescr, gravity, refStartPoint, refVelocity, startPoint, isOwnShoot=False, tracerCameraPos=Math.Vector3(0, 0, 0)):
        if shotID in self.__projectiles:
            ...
            proj['effectsData'] = {}
            projEffects.attachTo(proj['model'], proj['effectsData'], 'flying', stream=True)
        ...
        while True:
            ...
            for curCheckPoint in checkPoints:
                ...
                if testRes is not None:
                    ...
                    if distWater >= 0 and distWater < distStatic:
                        hitDirection = curCheckPoint - prevCheckPoint
                        hitDirection.normalise()
                        hitPoint = prevCheckPoint + hitDirection * distWater
                    return (hitPoint, (hitPoint[0] - r0[0]) / v0[0])
                elementLength = (curCheckPoint - prevCheckPoint).length
                if distWater >= 0 and distWater <= elementLength:
                    hitDirection = curCheckPoint - prevCheckPoint
                    hitDirection.normalise()
                    hitPoint = prevCheckPoint + hitDirection * distWater
                    return (
                     hitPoint, (hitPoint[0] - r0[0]) / v0[0])
...
def collideDynamic(startPoint, endPoint, exceptIDs):
    res = None
    ...
    for vehicleID in BigWorld.player().arena.vehicles.iterkeys():
        ...
        if vehicle is None or not vehicle.isStarted:
            continue
        collRes = vehicle.collideSegment(startPoint, endPoint)
        if collRes is None:
            continue
        dist = collRes[0]
        if dist < startPoint.distTo(endPoint):
            endPoint = startPoint + dir * dist
            res = (vehicle, dist, collRes[1], collRes[2])

    return res


def collideDynamicAndStatic(startPoint, endPoint, exceptIDs, collisionFlags=128):
    testResStatic = BigWorld.wg_collideSegment(BigWorld.player().spaceID, startPoint, endPoint, collisionFlags)
    testResDynamic = collideDynamic(startPoint, endPoint if testResStatic is None else testResStatic[0], exceptIDs)
    if testResStatic is None and testResDynamic is None:
        ...
        if distDynamic <= distStatic:
            ...
            dir.normalise()
            return (
             startPoint + distDynamic * dir,
             (
              testResDynamic[0], testResDynamic[2], testResDynamic[3]))
        return (
         testResStatic[0], None)
        return
