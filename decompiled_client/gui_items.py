...
# Compiled at: 2011-06-02 13:58:57
import BigWorld, math, pickle
...
from items.tankmen import TankmanDescr, getNationConfig, getSkillsConfig, ROLES, COMMON_SKILLS, SKILL_NAMES
...
class FittingItem(object):
    ...
    def getPriceByModifiers(self, sellPriceModifiers):
        return (
         BigWorld.player().shop.getSellPrice(self.priceOrder, sellPriceModifiers), 0)
    ...
    def getTableName(self, vehicle=None):
        ...
        if self.itemTypeName == ITEM_TYPE_NAMES[1]:
            ...
            damage = ('/').join([_[1] for shot in self.descriptor['shots']])
            piercingPower = ('/').join([_[2] for shot in self.descriptor['shots']])
            if vehicle is None:
                reloadTime = '%s' % BigWorld.wg_getNiceNumberFormat(self.descriptor['burst'][0] * 60.0 / (self.descriptor['reloadTime'] + self.descriptor['burst'][0] * self.descriptor['burst'][1]))
            else:
                guns = []
                ...
                from parameters import findMinMax, getMinMaxAsString
                reloadTime = {}
                for d in guns:
                    rT = d['burst'][0] * 60.0 / (d['reloadTime'] + d['burst'][0] * d['burst'][1])
        ...
        elif self.itemTypeName == ITEM_TYPE_NAMES[9]:
            return self.description
        if self.itemTypeName == ITEM_TYPE_NAMES[10]:
            damage = (BigWorld.wg_getIntegralFormat(round(self.descriptor['damage'][0] - self.descriptor['damage'][0] * self.descriptor['damageRandomization'])),
             BigWorld.wg_getIntegralFormat(round(self.descriptor['damage'][0] + self.descriptor['damage'][0] * self.descriptor['damageRandomization'])))
    ...
    def icon(self):
        ...
        if self.itemTypeName == ITEM_TYPE_NAMES[10]:
            return _ICONS_MASK[:-4] % {'type': (self.itemTypeName), 'subtype': '', 'unicName': (self.descriptor['icon'][0])}
        return _ICONS_MASK % {'type': (self.itemTypeName), 'subtype': '', 'unicName': (self.unicName.replace(':', '-'))}
    ...
    def iconContour(self):
        """
                ...
                """
        return _ICONS_MASK % {'type': (self.itemTypeName), 'subtype': 'contour/', 'unicName': (self.unicName.replace(':', '-'))}
...
class ShopItem(FittingItem):
    ...
    def __init__(self, itemTypeName, compactDescr, nation=None, priceOrder=(0, 0), hidden=False):
        ...
        if nation is None:
            if self.itemTypeName == ITEM_TYPE_NAMES[1]:
                ...
                raise 'vehicle without nation' or AssertionError
        return
        ...
        def getBuyResponse(code, itemInvID=None):
            if code >= 0:
                if callback is not None:
                    ...
                    callback((True, message, getPurchaseSysMessageType(buyPrice), component))
            else:
    ...
    def pack(self):
        return pickle.dumps([ShopItem, (self.itemTypeName, self.compactDescr, self.nation, self.priceOrder)])
...
class InventoryItem(FittingItem):
    ...
    def apply(self, callback, slotIdx=None, isUseGold=False):
        ...
        if self.itemTypeName == ITEM_TYPE_NAMES[1]:
            ...
            if self.itemTypeName == ITEM_TYPE_NAMES[9]:
                ...
                value = (
                 False, self._getMessage('apply_error_is_current'))
            else:
                if self.itemTypeName != ITEM_TYPE_NAMES[3]:
                    ...
                    if mayInstall[0]:
                        BigWorld.player().inventory.equip(g_currentVehicle.vehicle.inventoryId, self.compactDescr, (lambda code: self.__responseApply(code, callback)))
                        return
                    error = mayInstall[1].replace(' ', '_')
                    type = 'apply_error_' + error
                    ...
                    if mayInstall[0]:
                        BigWorld.player().inventory.equipTurret(g_currentVehicle.vehicle.inventoryId, self.compactDescr, 0, (lambda code: self.__responseApply(code, callback, 0)))
                        return
                    self.__applyTurret(mayInstall[1] == 'too heavy', callback)
                    return
                value = (
                 False, self._getMessage(type))
        elif g_currentVehicle.isLocked():
            value = (
             False, self._getMessage('apply_vehicle_locked'))
        else:
            value = (
             False, self._getMessage('apply_vehicle_broken'))
        if callback is not None:
            callback(value)
        return
    ...
    def __applyTurret(self, isToHeavy, callback):
        ...
        for iGun in iGuns:
            for gun in self.descriptor['guns']:
                if iGun.compactDescr == gun['compactDescr']:
                    ...
                    if mayInstall[0]:
                        BigWorld.player().inventory.equipTurret(g_currentVehicle.vehicle.inventoryId, self.compactDescr, gun['compactDescr'], (lambda code: self.__responseApply(code, callback, gun['compactDescr'])))
                        return
        ...
        if isToHeavy:
            value = (
             False, self._getMessage('apply_error_too_heavy'))
        else:
            value = (
             False, self._getMessage('apply_error_no_gun'))
        if callback is not None:
            callback(value)
        return

    def __responseApply(self, code, callback, gun=None, slotIdx=None, equipments=[]):
        if code >= 0:
            value = (
             True, self._getMessage('apply_success'))
            if equipments:
                pass
                ...
                if gun != 0:
                    value = (
                     True, self._getMessage('apply_success_gun_change', getDictDescr(gun)['userString']))
            else:
                g_currentVehicle.vehicle.descriptor.installComponent(self.compactDescr)
        else:
            LOG_ERROR('Server response en error for install item(%s - %s) operation code: %s' % (self.itemTypeName, self.descriptor['name'], code))
            value = (False, self._getMessage('server_apply_error'))
        if callback is not None:
            callback(value)
        return
    ...
    def __responseRemove(self, code, callback, slotIdx, equipments=[], isUseGold=False):
        ...
        if code >= 0:
            ...
            if self.isRemovable:
                value = (
                 True, self._getMessage('remove_success'))
            elif isUseGold:
                value = (
                 True, self._getMessage('remove_gold_success', cost))
            else:
                value = (
                 True, self._getMessage('destroy_success'))
        else:
        ...
        def getSellResponse(code):
            if code >= 0:
                if callback is not None:
                    message = ''
                    if self.itemTypeName == ITEM_TYPE_NAMES[1]:
                        message = self._getMessage('sell_success', formatPrice(sellPrice))
                    else:
    ...
    def setShellsList(self, shellsList=None):
        if shellsList is None:
            ...
            while len(shellsList) != 0:
                ...
                if shot is not None:
                    self.__shells.append(VehicleShellSlot(shot['shell'], cDescr, count, default, shot['piercingPower'], shot['shell']['damage']))
    ...
    def loadShells(self, shells, callback):
        ...
        def loadShellsResponse(responseCode):
            ...
            if responseCode < 0:
                ...
                response = (
                 True, '#system_messages:charge/success')
            if callback:
                callback(response)
            return
...
class InventoryTankman(InventoryItem):
    ...
    def efficiencyRoleLevel(self, vehicleDescr):
        (factor, addition) = self.descriptor.efficiencyOnVehicle(vehicleDescr) if self.isInTank else (1,
                                                                                                      0)
    ...
    def dropSkill(self, skillName, callback):
        ...
        if ready:
            assert hasattr(BigWorld.player(), 'inventory'), 'Request from inventory is not possible'
            BigWorld.player().inventory.dropTankmanSkill(self.inventoryId, skillName, dropSkillResponse)
            return
        else:
            if callback is not None:
                callback((False, message))
    ...
    def respec(self, vehTypeCompDescr, tmanCostTypeIdx, callback):
        ...
        def respecResponse(code):
            if code >= 0:
                callback((
                 True,
                 makeString('#system_messages:retrainingTankman/success') + formatTankmanPrice(upgradeParams, tmanCostTypeIdx)))
            else:
                callback((
                 False,
                 '#system_messages:retrainingTankman/server_error'))
            return
    ...
    def unload(self, callback):
        ...
        if ready:
            ...
            if berthsCount <= tankmanInBarracks:
                ...
                if slot is not None and vehicle.lock == LOCK_REASON.NONE:
                    assert hasattr(BigWorld.player(), 'inventory'), 'Request from inventory is not possible'
...
class VehicleShellSlot(VehicleItem):
    ...
    def getTableName(self):
        ...
        pPower = (
         BigWorld.wg_getIntegralFormat(round(self.piercingPower[0] - self.piercingPower[0] * self.descriptor['piercingPowerRandomization'])),
         BigWorld.wg_getIntegralFormat(round(self.piercingPower[0] + self.piercingPower[0] * self.descriptor['piercingPowerRandomization'])))
        damage = (
         BigWorld.wg_getIntegralFormat(round(self.damage[0] - self.damage[0] * self.descriptor['damageRandomization'])),
         BigWorld.wg_getIntegralFormat(round(self.damage[0] + self.damage[0] * self.descriptor['damageRandomization'])))
...
def getVehicleTypeFullName(vehicleType):
    ...
    tagsDump = [_[1] for tag in vehicleType.tags if typeInfo['tags'][tag]['userString'] != '']
...
def getVehicleParams():
    if g_currentVehicle.isPresent():
        vd = g_currentVehicle.vehicle.descriptor
        pPower = (
         BigWorld.wg_getIntegralFormat(round(vd.shot['piercingPower'][0] - vd.shot['piercingPower'][0] * vd.shot['shell']['piercingPowerRandomization'])),
         BigWorld.wg_getIntegralFormat(round(vd.shot['piercingPower'][0] + vd.shot['piercingPower'][0] * vd.shot['shell']['piercingPowerRandomization'])))
        damage = (
         BigWorld.wg_getIntegralFormat(round(vd.shot['shell']['damage'][0] - vd.shot['shell']['damage'][0] * vd.shot['shell']['damageRandomization'])),
         BigWorld.wg_getIntegralFormat(round(vd.shot['shell']['damage'][0] + vd.shot['shell']['damage'][0] * vd.shot['shell']['damageRandomization'])))
        data = list()
        ...
        data.extend(('reloadTime', BigWorld.wg_getNiceNumberFormat(vd.gun['burst'][0] * 60.0 / (vd.gun['reloadTime'] + vd.gun['burst'][0] * vd.gun['burst'][1]))))
...
def isModuleFitVehicle(module, vehicle, price, money, unlocks, positionIndex=0):
    installPosible = True
    if isinstance(module, ShopItem):
    ...
    if module.itemTypeName == ITEM_TYPE_NAMES[3]:
            ...
            for m in unlocks:
                if m.index != positionIndex:
                    if m is not module:
                        ...
                        reason = installPosible or 'not with installed equipment'
                        break

        else:
            (installPosible, reason) = vehicle.descriptor.mayInstallComponent(module.compactDescr)
            if installPosible:
                (oldModule,) = vehicle.descriptor.installComponent(module.compactDescr)
                for m in vehicle.equipments:
                    if m:
                        e = getDictDescr(m)
                        (installPosible, reason) = e.checkCompatibilityWithVehicle(vehicle.descriptor)
                        if not installPosible:
                            if reason == 'not for current vehicle':
        ...
        if not installPosible:
            ...
            return (
             installPosible, '#menu:moduleFits/' + reason, '#tooltips:moduleFits/' + reason + '/body')
    return (
     True, '', '')
